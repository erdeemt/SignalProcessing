...
Script 03: Offline Inference Test with GUI
...
def discover_models(models_dir="./models", results_dir="./results"):
    """
    02_training'in ürettiği modelleri keşfeder.
    Dosya ismi: 211805048_..._{feature_family}_{win}_{hop}_{LANG}_best_model.joblib
    Master CSV: 211805048_..._all_results.csv içinde 'dataset' kolonuyla eşlenir.
    """
    models = []
    ...
    # Master results (Accuracy, F1, Response_time_ms vs.)
    ...
    for model_path in model_files:
        ...
        if len(parts) >= 4 and parts[0] == 'time' and parts[1] == 'freq':
            ...
            window_length = parts[1] if len(parts) > 1 else ""
            hop = parts[2] if len(parts) > 2 else ""
        ...
        #Infos abaout models written to params.json
        language = params.get('language', None)

        # get perf metrics from master.csv
        accuracy = 0.0
        f1_score = 0.0
        response_time = 0.0
        model_name = params.get('best_model', 'Unknown')

        if results_df is not None:
            matching_row = results_df[results_df['dataset'] == dataset_key]
        ...
        lang_str = language if language in ['TR', 'EN'] else '??'
        display_name = (
            f"{lang_str} | {feature_family} | {window_length} | "
...
def load_ground_truth(audio_path):
    ...
    if os.path.exists(csv_path):
        try:
            ...
            # ✅ Original SR'ı bul (audio dosyasından)
            y_orig, orig_sr = librosa.load(audio_path, sr=None, mono=True)
            
            # ✅ Sample → Time dönüşümü yap (01_extraction ile aynı)
...
def process_audio_file(audio_path, model_info, model, scaler, needs_scaling, language):
    """
    Ses dosyasını kaydırmalı pencere ile işler, 02_training'le uyumlu feature çıkartır
    ve modeli kullanarak tahmin yapar.
    """
    # Load audio with pre-emphasis
    ...
    try:
        window_length_s = float(
            model_info['window_length'].replace('s', '').replace('p', '.')
        )
        hop_s = float(
            model_info['hop'].replace('s', '').replace('p', '.')
        )
    except ValueError as e:
        print(f"Error parsing window parameters: {e}")
        print(f"model_info: {model_info}")
        raise ValueError(
            f"Invalid window/hop format in model_info: "
            f"window={model_info.get('window_length')}, "
            f"hop={model_info.get('hop')}"
        )
    ...
    while start_sample + window_samples <= len(y):
        ...
        windows.append({
            'data': y[start_sample:end_sample],
    ...
    for idx, window in enumerate(windows):
        ...
        # Extract features
        features = extract_features(window['data'], sr, model_info['feature_family'])
        X = pd.DataFrame([features], dtype=np.float32)

        # Align
        feature_names = model_info['params'].get('feature_names', None)
        ...
        response_time = (t1 - t0) * 1000.0

        # Match ground truth: According to center of window
        true_label = 0 if gt_df is not None else None
        if gt_df is not None:
            # Calculate center as a second
            center_time = 0.5 * (window['start_time'] + window['end_time'])
            ...
            for _, row in gt_df.iterrows():
                ...
                gt_start_time = row['start_time']
                gt_end_time = row['end_time']

            
                if gt_start_time <= center_time < gt_end_time:
        ...
        predictions.append({
            ...
            'correct': (true_label == int(y_pred)) if true_label is not None else None,
...
class OfflineTestGUI:
    ...
    def _select_best_by_language(self, target_lang):
        """İç fonksiyon: verilen dil (TR/EN) için en iyi modeli seçer."""
        if not self.available_models:
    ...
    def _process_thread(self, audio_path, lang):
        try:
            ...
            self.results_text.insert(
                tk.END,
                f"Processing: {os.path.basename(audio_path)}\n"
            )
            self.results_text.insert(tk.END, f"Language (audio): {lang}\n")
            self.results_text.insert(
                tk.END,
                f"Model language: {self.selected_model_info.get('language', 'N/A')}\n"
            )
