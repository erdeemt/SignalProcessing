"""
SCRIPT 04 (GUI + REALTIME) - SEGMENT-BASED VERSION 
...
class RealTimeGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Real-Time Voice Command Recognition (SEGMENT-BASED)")
    ...
    def _build_gui(self):
        ...
        # --- Mikrofon seçimi (sadeleştirildi, test butonu yok) ---
        ...
        # Threshold slider kaldırıldı → energy_threshold koddan ayarlanıyor
    ...
    def _discover_models(self):
        ...
        self.model_files = glob("./models/*_best_model.joblib")
    ...
    def _process_segment(self, segment_audio):
        """
        Segment üzerinde sliding window ile prediction yap.
        Kısa segmentler için window_length'ten küçükse zero-padding yapıyoruz.
        """
        try:
            ...
            # Segment, pencere uzunluğundan kısaysa padle
            ...
            self._write_async(f"  Windows analyzed: {len(predictions)}\n")
            self._write_async(f"  Predictions: {dict(pred_counts)}\n")
            ...
            if len(non_bg_preds) >= len(predictions) * 0.3:
                ...
                avg_prob = np.mean(
                    [probabilities[i] for i, p in enumerate(predictions) if p == final_pred]
                )

                cmd_name = self._get_command_name(final_pred)

                self._write_async(
                    f"  ✅ COMMAND DETECTED: #{final_pred} - {cmd_name} "
                    f"(Conf: {confidence:.0f}%, Prob: {avg_prob:.2f})\n\n"
                )
                self.total_predictions += 1

            else:
                self._write_async(f"  ⚪ Background/Noise (no clear command)\n\n")

        except Exception as e:
            self._write_async(f"  ❌ Processing error: {str(e)}\n\n")
            import traceback
            traceback.print_exc()
